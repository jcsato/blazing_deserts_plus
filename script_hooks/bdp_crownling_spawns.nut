::mods_hookNewObject("entity/world/entity_manager", function(em) {
	...
	::mods_override(em, "manageAIMercenaries", function() {
		...
		if (m.Crownlings.len() < ::BDP.Crownlings.MaxParties || (World.FactionManager.isHolyWar() && m.Crownlings.len() < ::BDP.Crownlings.HolyWarMaxParties)) {
			...
			local mercList = clone m.Crownlings;
